-- Complete optional Bulk onboarding and activation in one transaction. Existing
-- profiles remain valid and are never rewritten by this RPC.
ALTER TABLE public.bulk_targets
  ADD CONSTRAINT bulk_targets_onboarding_payload_ck CHECK (
    CASE WHEN payload ? 'startWeight'
    ...
    AND CASE WHEN payload ? 'targetWeight'
      THEN jsonb_typeof(payload->'targetWeight') = 'number'
        AND (payload->>'targetWeight')::numeric > 0
        AND (payload->>'targetWeight')::numeric <= 450
      ELSE true END
    ...
    AND CASE WHEN payload ? 'targetWeeklyGainKg'
      THEN jsonb_typeof(payload->'targetWeeklyGainKg') = 'number'
        AND (payload->>'targetWeeklyGainKg')::numeric BETWEEN 0.05 AND 1.5
      ELSE true END
    ...
    AND CASE WHEN payload ? 'availableEquipment'
      THEN jsonb_typeof(payload->'availableEquipment') = 'array'
        ...
        AND NOT (
          payload->'availableEquipment' ? 'bodyweight_only'
...
BEGIN
  ...
  -- Serialize duplicate taps for this account before checking for an existing plan.
  ...
  IF _target_weight_kg IS NULL OR _target_weight_kg <= _current_weight_kg OR _target_weight_kg > 450
    THEN RAISE EXCEPTION 'Invalid target weight'; END IF;
  IF _target_weekly_gain_kg IS NULL OR _target_weekly_gain_kg < 0.05 OR _target_weekly_gain_kg > 1.5
    THEN RAISE EXCEPTION 'Invalid weekly gain'; END IF;
  IF _experience_level IS NULL OR _experience_level NOT IN ('beginner', 'intermediate', 'advanced')
    THEN RAISE EXCEPTION 'Invalid experience level'; END IF;
  IF _training_days_per_week IS NULL OR _training_days_per_week NOT IN (2, 3, 4, 5, 6)
    THEN RAISE EXCEPTION 'Invalid training days'; END IF;
  IF _training_setup_preference IS NULL OR _training_setup_preference NOT IN ('generated', 'tempo_preset', 'custom')
...
REVOKE ALL ON FUNCTION public.complete_bulk_onboarding(numeric,numeric,numeric,text,integer,text[],text,integer,integer,integer,integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.complete_bulk_onboarding(numeric,numeric,numeric,text,integer,text[],text,integer,integer,integer,integer) TO authenticated;

-- The Segment 1 placeholder activation endpoint must no longer bypass required onboarding.
REVOKE EXECUTE ON FUNCTION public.activate_my_bulk() FROM authenticated;
GRANT EXECUTE ON FUNCTION public.activate_my_bulk() TO service_role;
